
/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.proyectofinal3;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.File;
import java.io.FileNotFoundException;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import java.time.LocalDate;
import java.util.ArrayList;
import java.util.logging.Level;
import java.util.logging.Logger;

/**
 *
 * @author fernada
 */
public class Archivo {
    public static void escribir(String ruta, String contenido, boolean agregar) {
    File archivo = new File(ruta);
    boolean archivoNuevo = !archivo.exists();

    try (BufferedWriter writer = new BufferedWriter(new FileWriter(archivo, agregar))) {
        // Si es un archivo nuevo y se está escribiendo por primera vez, agregar encabezado
        if (!contenido.trim().isEmpty()) {
           writer.write(contenido.trim());
          writer.newLine();
       }

        writer.write(contenido);
        writer.newLine();  
    } catch (IOException ex) {
        Logger.getLogger(Archivo.class.getName()).log(Level.SEVERE, null, ex);
    }
}

         
         public static void main(String[] args) { 
         
       
        ArrayList<Cupon> cupon = leer("rutaRelativa.csv");
        
        cupon.forEach((c)->{
            System.out.print(c.getCodigo());
           System.out.print(",");
    System.out.print(c.getDescuento());
    System.out.print(",");
    System.out.print(c.getTipo());
    System.out.print(",");
    System.out.print(c.getFechaVencimiento());
    System.out.println("");
        });
        
    }
    
    public static ArrayList<Cupon> leer(String ruta){
    
        ArrayList<Cupon> contenido = new ArrayList<>();
        
        
        try {
            File archivo = new File(ruta);
            FileReader leer = new FileReader(archivo);
            BufferedReader bloque = new BufferedReader(leer);
            String linea = bloque.readLine();
            while(linea !=null){
                String[] cupon = linea.split("\\|");
                Cupon c = new Cupon();
            c.setCodigo(cupon[0].trim());
            c.setDescuento(Double.parseDouble(cupon[1].trim()));
            c.setTipo(cupon[2].trim());
            c.setFechaVencimiento(LocalDate.parse(cupon[3].trim()));
                contenido.add(c);
                linea = bloque.readLine();
            }
            leer.close();
        } catch (FileNotFoundException ex) {
            Logger.getLogger(Archivo.class.getName()).log(Level.SEVERE, null, ex);
        } catch (IOException ex) {
            Logger.getLogger(Archivo.class.getName()).log(Level.SEVERE, null, ex);
        }
     
        return contenido;
        
    }
}
